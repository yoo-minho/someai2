...
const isSummaryLargeImg = computed(() => ogData.value.twitterCard === 'summary_large_image');
...
<template>
    <template v-if="stepNo > 0">
        <h2 class="text-2xl font-bold text-primary-400 mb-2"># Step 1. Choose Sharing App</h2>
        <UContainer class="mb-6 sm:p-0 p-0 flex gap-3">
            <template v-for="appId in ['KakaoTalk', 'Facebook', 'Slack']">
                <UButton :label="appId" :color="activeApp === appId ? 'primary' : 'gray'" @click="clickApp(appId)" />
            </template>
        </UContainer>
    </template>

    <template v-if="stepNo > 1">
        <h2 class="text-2xl font-bold text-primary-400 mb-2"># Step 2. Enter Url</h2>
        ...
        <div v-else class="mt-3">
            <UContainer v-if="activeApp === 'KakaoTalk'" class="flex justify-center">
                <div class="w-400px h-294px border-1px border-solid rounded-8px bg-white box-content"
                    style="border-color: #e5e5e5">
                    <div v-if="thumbnailUrl" class="w-full h-210px bg-cover bg-no-repeat bg-center rounded-t-8px "
                        :style="{ 'background-image': `url(${thumbnailUrl})` }">
                    </div>
                    <div v-else-if="ogData.ogUrl === 'ogUrl'"
                        class="w-400px h-210px text-white bg-black flex items-center justify-center rounded-t-8px">
                        NO THUMBNAIL
                    </div>
            ...
            <UContainer v-if="activeApp === 'Facebook'" class="flex justify-center">
                <div class="w-400px border-1 border-solid bg-white box-content" style="border-color: #dadde1">
                    <div v-if="thumbnailUrl" class="w-full aspect-[1.91/1] bg-cover bg-no-repeat bg-center"
                        :style="{ 'background-image': `url(${thumbnailUrl})` }">
                    </div>
                    <div v-else-if="ogData.ogUrl === 'ogUrl'"
                        class="w-400px aspect-[1.91/1] text-white bg-black flex items-center justify-center">NO
                        THUMBNAIL
                    </div>
            ...
            <UContainer v-if="activeApp === 'Slack'" class="flex justify-center bg-white p-4">
                <div class="w-400px border-l-4px border-solid px-12px text-15px leading-22px "
                    style="border-color: rgb(221,221,221); color: rgb(29,28,29);">
                    <div class="flex">
                        <img v-if="ogData.favicon" :src="ogData.requestUrl + '/' + ogData.favicon" width="16"
                            height="16" class="mr-8px mb-3px object-contain">
                        <span class="font-bold">{{ domain }}</span>
                    </div>
                    <div style="color: rgb(18,100,163)" class="font-bold">{{ ogData.ogTitle }}</div>
                    <template v-if="isSummaryLargeImg && thumbnailUrl">
                        <div>{{ ogData.ogDescription }}<span class="ml-1">(nkB)</span></div>
                        <div class="w-360px h-189px bg-contain bg-no-repeat bg-center mt-5px rounded-8px"
                            :style="{ 'background-image': `url(${thumbnailUrl})`, 'box-shadow': 'inset 0 0 0 1px #0000001a' }">
                        </div>
                    </template>
    ...
    <template v-if="stepNo > 2">
        <h2 class="text-2xl font-bold text-primary-400 mb-2 mt-6"># Step 3. MakeUp Url</h2>
        <template v-if="state2.newTitle === ''">
            wait...
        </template>
        <template v-else>
            <UForm :schema="schema2" :state="state2" class="text-2xl mt-3 flex-col flex gap-3">
                <UFormGroup label="Title" name="newTitle">
                    <UInput v-model="state2.newTitle" placeholder="Enrer New Title" size="lg" />
                </UFormGroup>
                <UFormGroup label="Description" name="newDesc">
                    <UInput v-model="state2.newDesc" placeholder="Enrer New Desc" size="lg" />
                </UFormGroup>
            </UForm>
            ...
            <div class="mt-4">
                ...
                <UContainer v-if="activeApp === 'Facebook'" class="flex justify-center">
                    <div class="w-400px border-1 border-solid bg-white box-content" style="border-color: #dadde1">
                        ...
                        <div class="px-12px py-10px leading-20px">
                            ...
                            <div class="text-17px font-[600]" style="color:rgb(5, 5, 5)">{{ state2.newTitle }}
                            </div>
                            <div class="text-15px font-[400] text-ellipsis overflow-hidden whitespace-nowrap"
                                style="color:rgb(101, 103, 107);">
                                {{ state2.newDesc }}
                ...
                <UContainer v-if="activeApp === 'Slack'" class="flex justify-center bg-white p-4">
                    <div class="w-400px border-l-4px border-solid px-12px text-15px leading-22px "
                        ...
                        <div style="color: rgb(18,100,163)" class="font-bold">{{ state2.newTitle }}</div>
                        <template v-if="isSummaryLargeImg && thumbnailUrl">
                            <div>{{ state2.newDesc }}<span class="ml-1">(nkB)</span></div>
                            <div class="w-360px h-189px bg-contain bg-no-repeat bg-center mt-5px rounded-8px"
                                :style="{ 'background-image': `url(${thumbnailUrl})`, 'box-shadow': 'inset 0 0 0 1px #0000001a' }">
                            </div>
                        </template>
            ...
            <h2 class="text-2xl font-bold text-primary-400 mb-2 mt-6"># Step 4. Save & Share</h2>
            <UForm :schema="schema2" :state="state" class="flex gap-3 text-2xl" @submit="onSubmit2">
                <UFormGroup name="url" class="flex-1 text-2xl">
