...
<template>
  <div class="w-full h-full flex flex-col justify-center">
    <div class="flex justify-center">
      <div>
        <div v-if="ogImage" class="bg-cover bg-center" :style="{
        ...
        <div v-if="preview" class="layer">
          ...
          <Banner :title="_title" :thumbUrl="convertThumbUrl" style="position: relative" />
        </div>
      </div>
    </div>
  </div>
...
.container {
  position: relative;
  overflow: hidden;
  /* 요소 내부의 내용이 넘치는 경우를 방지하기 위해 */
}

.layer {
  position: absolute;
  width: 100%;
  /* 이미지의 크기를 scale(0.5)로 축소할 것이므로 200% */
  height: 100%;
  /* 이미지의 크기를 scale(0.5)로 축소할 것이므로 200% */
